...
from mcp.schema import MCPSchemaType, MCPParameterSchema, MCPOutputSchema, standard_output_schema
from mcp.state_machine import AgentState
from mcp.registry import MCPToolDefinition, MCPToolRegistry
...
# ── Agentic VLM tool subset ───────────────────────────────────────────────
# These are the utility tools exposed to the VLM during agentic monitoring.
AGENTIC_TOOL_DEFS = [
...
def get_agentic_tool_schemas() -> List[Dict[str, Any]]:
    """Return tool schemas for VLM agentic analysis prompts."""
    schemas = []
    for t in AGENTIC_TOOL_DEFS:
        fn = t.to_json_schema()["function"]
        schemas.append({
            "name": fn["name"],
