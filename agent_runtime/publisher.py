...
class CameraFeedPublisher:
    ...
    def __init__(
        ...
        logger.info(
            f"CameraFeedPublisher initialized: camera={camera_index}, resolution={width}x{height}, fps={fps}"
        )
    ...
    def subscribe(
        ...
        with self._lock:
            ...
            logger.info(
                f"Subscriber '{subscriber_id}' added. Total subscribers: {self.stats['subscribers_count']}"
            )
            return True
    ...
    def unsubscribe(self, subscriber_id: str) -> bool:
        ...
        with self._lock:
            ...
            logger.info(
                f"Subscriber '{subscriber_id}' removed. Total subscribers: {self.stats['subscribers_count']}"
            )
            return True

    def get_frame(
        self, subscriber_id: str, timeout: float = 1.0
        ...
        while self.is_running:
            ...
            try:
                ...
                encode_success, buffer = cv2.imencode(
                    ".jpg", raw_frame, [cv2.IMWRITE_JPEG_QUALITY, 85]
                )
...
def get_camera_publisher(
    camera_index: int = 0, width: int = 640, height: int = 480, fps: int = 30
) -> CameraFeedPublisher:
    """
