...
def _cleanup_old_sessions_legacy():
    ...
    with agent_sessions_lock:
        ...
        for sid in sessions_to_remove:
            ...
            try:
                from agent.tools import cleanup_session_storage
...
def _get_session_warnings(session_id: str) -> Dict[str, Any]:
    ...
    """
    try:
        from mcp.session import check_session_warnings, is_session_over_hard_limit
        from mcp.session_tracking import WarningLevel
...
def _record_session_usage(
    ...
    """
    try:
        from mcp.session import get_session
...
def agent_chat():
    ...
    try:
        ...
        try:
            from agent.prompts import process_chat_message, check_agent_enabled
            from agent.tools import get_session_storage_path, check_session_storage_limit
        ...
        # Debug: Log session info
        logger.info(f"📝 Agent chat request - session_id: {session_id}, message preview: {user_message[:50]}...")
        ...
        try:
            from mcp.session import get_or_create_session, SessionCapacityError
        ...
        with agent_sessions_lock:
            if session_id in agent_sessions:
                ...
                try:
                    from mcp.session import get_session_status
...
def _generate_sse_events(message: str, session_id: str, image_path: str = None) -> Generator[str, None, None]:
    ...
    """
    try:
        from agent.prompts import process_chat_message_stream, check_agent_enabled
        from agent.tools import get_session_storage_path
    except ImportError as e:
        logger.error(f"Failed to import agent modules: {e}")
    ...
    try:
        ...
        for event in process_chat_message_stream(message, history, session_id=session_id, image_path=image_path):
            ...
            if event_type == "token":
                ...
                yield f"event: error\ndata: {json.dumps({'error': event.get('error', 'Unknown error')}, ensure_ascii=False)}\n\n"
                return
...
def agent_chat_stream():
    ...
    Returns a stream of events for real-time token-by-token response rendering.
    ...
    try:
        ...
        # Handle image path if present (for multipart, need separate endpoint)
...
def agent_status():
    ...
    try:
        from agent.prompts import check_agent_enabled, get_backend_info
...
def get_session_status(session_id: str):
    ...
    """
    try:
        from mcp.session import get_session_status as mcp_get_session_status
...
def get_session_configuration():
    ...
    """
    try:
        from mcp.session_config import get_session_config
        ...
        return jsonify({
            ...
            "config": {
                "limits": {
                    "max_storage_mb": SESSION_STORAGE_LIMIT_MB if 'SESSION_STORAGE_LIMIT_MB' in dir() else 30.0,
                },
                "session": {
