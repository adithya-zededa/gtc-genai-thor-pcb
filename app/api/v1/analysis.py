...
def _capture_current_frame():
    ...
    try:
        ...
        if not latest_frame or not latest_frame.image_b64:
            return None, (
                jsonify({"success": False, "error": "No frames available from camera"}),
                500,
            )
        ...
        if frame is None:
            return None, (
                jsonify({"success": False, "error": "Failed to decode camera frame"}),
                500,
            )
        ...
        return None, (
            jsonify({"success": False, "error": f"Camera access failed: {err}"}),
            500,
        )
...
def analyze_prompt():
    ...
    try:
        ...
        if task_type is None:
            return (
                jsonify(
                    {"success": False, "error": f"Invalid task_type: {task_type_str}"}
                ),
                400,
            )
        ...
        if task_type == TaskType.CUSTOM and not custom_prompt:
            return (
                jsonify(
                    {"success": False, "error": "Custom task requires a custom_prompt"}
                ),
                400,
            )
        ...
        if result is None:
            return (
                jsonify(
                    {"success": False, "error": "Analysis failed - no result returned"}
                ),
                500,
            )
...
def analyze_uploaded_image():
    ...
    try:
        ...
        return (
            jsonify({"success": False, "error": f"Failed to process image: {str(e)}"}),
            400,
        )
    ...
    try:
        ...
        if event is None:
            return (
                jsonify({"success": False, "error": "Analysis returned no result."}),
                500,
            )
        ...
        return jsonify(
            {
                ...
                "result": {
                    "task_type": task_type_str
                    or (task_type.value if task_type else "unknown"),
