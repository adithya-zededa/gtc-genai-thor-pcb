...
class AlertManager:
    ...
    def send_email_alert(
        ...
        Args:
            ...
            rule: The rule configuration for this alert.
            image_data: Optional raw image bytes to attach (preferred over event.image_path).
        ...
        try:
            ...
            # Attach image - prefer raw bytes over file path
            image_attached = False
            
            # Source 1: image_data bytes passed directly (preferred)
            if image_data and isinstance(image_data, bytes):
                ...
                logger.info("Attached detection image (%d bytes) to email alert", len(image_data))
                image_attached = True
            
            # Source 2: image_path from event (fallback)
            if not image_attached and event.image_path:
                try:
                    ...
                    if image_path.exists():
                        ...
                        logger.info("Attached detection image %s to email alert", image_path)
                        image_attached = True
                    else:
